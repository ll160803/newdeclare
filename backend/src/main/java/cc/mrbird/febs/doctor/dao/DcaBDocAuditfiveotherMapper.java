...
@Update(" update dca_b_doc_auditfiveother set IS_DELETEMARK=0 where user_account=#{useraccount}  and (state=0 or state=2)")
        void deleteByAccount(@Param("useraccount") String useraccount);
@Select(" select IFNULL(max(display_index),0) As maxIndex from  dca_b_doc_auditfiveother  where user_account=#{useraccount} ")
