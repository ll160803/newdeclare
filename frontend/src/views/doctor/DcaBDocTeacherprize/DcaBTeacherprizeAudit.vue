...
import DcaBTeacherprizeDone from './DcaBTeacherprizeDone'
...
export default {
  ...
  components: { DcaBTeacherprizeDone, AuditUserInfo },
  mounted () {
    this.search()
  },
   props: {
  ...
  methods: {
    ...
    handleAuditNext (record) {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          that.loading = true
          that.$post('dcaBDocTeacherprize/updateNew', {
            jsonStr: jsonStr,
            state: 1
          }).then(() => {
            //this.reset()
    ...
    fetch (params = {}) {
      ...
      this.loading = true
      this.$get('dcaBDocTeacherprize/audit', {
        ...params,
        state: 1
      }).then((r) => {
        let data = r.data
