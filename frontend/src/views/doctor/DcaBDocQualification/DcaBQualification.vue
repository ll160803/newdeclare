  ...
  methods: {
    ...
    handleSave () {
      ...
      dataSource.forEach(element => {
        if (element.qualificationName != '' || element.qualificationDate != '' || element.qualificationCode != '' || element.qualificationGrade != '') {
      ...
      if (dataAdd.length === 0) {
        ...
        this.loading = true
        this.$post('dcaBDocQualification/addNew', {
          jsonStr: jsonStr,
          state: 0
        }).then(() => {
          // this.reset()
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          if (dataAdd.length === 0) {
            ...
            let jsonStr = JSON.stringify(dataAdd)
            that.loading = true
            that.$post('dcaBDocQualification/addNew', {
              jsonStr: jsonStr,
              state: 1
            }).then(() => {
              //this.reset()
    ...
    fetch () {
      this.$get('dcaBDocQualification/custom', {
      }).then((r) => {
        let data = r.data
