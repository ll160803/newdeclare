    ...
    <a-spin :spinning="loading">
      <a-card title="个人思想政治表现">
        ...
        >
          ...
          ></a-textarea>
          <a-textarea
            :value="dcaBPolitalshow.auditSuggestion"
...
import DcaBPolitalshowDone from './DcaBPolitalshowDone'
...
export default {
  ...
  components: { DcaBPolitalshowDone },
  mounted () {
    this.fetch()
  },
  methods: {
    ...
    inputChange (value, filedName) {
      this.dcaBPolitalshow[filedName] = value
    },
    onIsUseChange (e, filedName) {
      this.dcaBPolitalshow[filedName] = e.target.checked;
    },
    OpenAudit (record) {
    ...
    handleAudit () {
      ...
      this.$confirm({
        ...
        centered: true,
        onOk () {
          let jsonStr = JSON.stringify(that.dcaBPolitalshow)
          that.loading = true
          that.$post('dcaBDocPolitalshow/updateNew', {
            jsonStr: jsonStr,
            state: 3
          }).then(() => {
            //this.reset()
    ...
    fetch (params = {}) {
      ...
      this.loading = true
      this.$get('dcaBDocPolitalshow/audit', {
        ...params,
        state: 1
      }).then((r) => {
        let data = r.data
