...
import DcaBUndergraduateDone from './DcaBUndergraduateDone'
...
export default {
  ...
  methods: {
    ...
    handleAudit (record) {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          that.loading = true
          that.$post('dcaBDocUndergraduate/updateNew', {
            jsonStr: jsonStr,
            state: 3
          }).then(() => {
            //this.reset()
    ...
    fetch (params = {}) {
      ...
      this.loading = true
      this.$get('dcaBDocUndergraduate/audit', {
        ...params,
        state: 1
      }).then((r) => {
        let data = r.data
