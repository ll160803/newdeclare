...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            setFormValues ({...dcaBYoungprize}) {
                let fields = [       'prizeName'    , 'prizeJb'    , 'prizeGrade'    , 'ranknum'    , 'prizeDate'               ]
                let fieldDates = [            'prizeDate'               ]
                Object.keys(dcaBYoungprize).forEach((key) => {
                    if (fields.indexOf(key) !== -1) {
                    this.form.getFieldDecorator(key)
                    let obj = {}
                    if(fieldDates.indexOf(key)!==-1)
                    {
                        if(dcaBYoungprize[key]!=='') {
                            obj[key] = moment(dcaBYoungprize[key])
                        }
                        else
                        {
                            obj[key] =''
                        ...
                        obj[key] = dcaBYoungprize[key]
                    }
                    this.form.setFieldsValue(obj)
                }
            })
                this.dcaBYoungprize. id                                             = dcaBYoungprize. id                                                        },
            handleSubmit () {
                this.form.validateFields((err, values) => {
                    if (!err) {
                    let dcaBYoungprize = this.form.getFieldsValue()
                    dcaBYoungprize. id                                            = this.dcaBYoungprize.id
                    this.$put('dcaBDocYoungprize', {
                        ...dcaBYoungprize
                    }).then(() => {
                        this.reset()
                        this.$emit('success')
