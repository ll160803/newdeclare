...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            setFormValues ({...dcaBSchoolprize}) {
                let fields = [       'prizeName'    , 'prizeGrade'    , 'prizeDate'    , 'ranknum'               ]
                let fieldDates = [          'prizeDate'                ]
                Object.keys(dcaBSchoolprize).forEach((key) => {
                    if (fields.indexOf(key) !== -1) {
                    this.form.getFieldDecorator(key)
                    let obj = {}
                    if(fieldDates.indexOf(key)!==-1)
                    {
                        if(dcaBSchoolprize[key]!=='') {
                            obj[key] = moment(dcaBSchoolprize[key])
                        }
                        else
                        {
                            obj[key] =''
                        ...
                        obj[key] = dcaBSchoolprize[key]
                    }
                    this.form.setFieldsValue(obj)
                }
            })
                this.dcaBSchoolprize. id                                           = dcaBSchoolprize. id                                                      },
            handleSubmit () {
                this.form.validateFields((err, values) => {
                    if (!err) {
                    let dcaBSchoolprize = this.form.getFieldsValue()
                    dcaBSchoolprize. id                                          = this.dcaBSchoolprize.id
                    this.$put('dcaBDocSchoolprize', {
                        ...dcaBSchoolprize
                    }).then(() => {
                        this.reset()
                        this.$emit('success')
