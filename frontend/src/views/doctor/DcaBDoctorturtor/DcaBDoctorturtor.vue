  ...
  methods: {
    ...
    handleSave () {
      ...
      dataSource.forEach(element => {
        if (element.turtorType != '' || element.turtorDate != '' || element.tutorContent != '') {
      ...
      if (dataAdd.length === 0) {
        ...
        this.loading = true
        this.$post('dcaBDocDoctorturtor/addNew', {
          jsonStr: jsonStr,
          state: 0
        }).then(() => {
          // this.reset()
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          if (dataAdd.length === 0) {
            ...
            let jsonStr = JSON.stringify(dataAdd)
            that.loading = true
            that.$post('dcaBDocDoctorturtor/addNew', {
              jsonStr: jsonStr,
              state: 1
            }).then(() => {
              //this.reset()
    ...
    fetch () {
      this.$get('dcaBDocDoctorturtor/custom', {
      }).then((r) => {
        let data = r.data
