<template>
  <a-card class="card-area" title="任现职以来承担的本科教学改革及建设项目">
    <div>
      <a-button
        @click="handleAdd"
        type="primary"
  ...
  methods: {
    ...
    handleSave () {
      ...
      if (dataAdd.length < 0) {
        ...
        this.loading = true
        this.$post('dcaBDocInnovatebuild/addNew', {
          jsonStr: jsonStr,
          state: 0
        }).then(() => {
          // this.reset()
    ...
    handleSubmit () {
      ...
      const dataSource = dataSourceAll.filter(p=>p.state==0 ||p.state==2)
          ...
          if (dataAdd.length < 0) {
            ...
            let jsonStr = JSON.stringify(dataAdd)
            that.loading = true
            that.$post('dcaBDocInnovatebuild/addNew', {
              jsonStr: jsonStr,
              state: 1
            }).then(() => {
              //this.reset()
    ...
    fetch () {
      this.$get('dcaBDocInnovatebuild/custom', {
      }).then((r) => {
        let data = r.data
