  ...
  methods: {
    ...
    handleSave () {
      ...
      dataSource.forEach(element => {
        if (element.academicName != '' || element.academicDate != '' || element.academicContent != '') {
      ...
      if (dataAdd.length === 0) {
        ...
        this.loading = true
        this.$post('dcaBDocAcademic/addNew', {
          jsonStr: jsonStr,
          state: 0
        }).then(() => {
          // this.reset()
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          if (dataAdd.length === 0) {
            ...
            let jsonStr = JSON.stringify(dataAdd)
            that.loading = true
            that.$post('dcaBDocAcademic/addNew', {
              jsonStr: jsonStr,
              state: 1
            }).then(() => {
              //this.reset()
    ...
    fetch () {
      this.$get('dcaBDocAcademic/custom', {
      }).then((r) => {
        let data = r.data
