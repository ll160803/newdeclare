  ...
  methods: {
    ...
    handleSave () {
      ...
      dataSource.forEach(element => {
        if (element.fileId != '' || element.fileUrl != '' || element.paperName != '' || element.journalName != '' || element.journalCode != '' || element.paperPublishdate != '' || element.paperShoulu != '' || element.paperCause != '' || element.isBest != '' || element.otherTimes != '' || element.authorRank != '') {
      ...
      if (dataAdd.length < 0) {
        ...
        this.loading = true
        this.$post('dcaBDocPaperspublish/addNew', {
          jsonStr: jsonStr,
          state: 0
        }).then(() => {
          // this.reset()
    ...
    handleSubmit () {
      ...
      const dataSource = dataSourceAll.filter(p=>p.state==0 ||p.state==2)
          ...
          if (dataAdd.length < 0) {
            ...
            let jsonStr = JSON.stringify(dataAdd)
            that.loading = true
            that.$post('dcaBDocPaperspublish/addNew', {
              jsonStr: jsonStr,
              state: 1
            }).then(() => {
              //this.reset()
    ...
    fetch () {
      this.$get('dcaBDocPaperspublish/custom', {
      }).then((r) => {
        let data = r.data
