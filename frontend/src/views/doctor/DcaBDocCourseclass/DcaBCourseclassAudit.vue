...
import DcaBCourseclassDone from './DcaBCourseclassDone'
...
export default {
  ...
  components: { DcaBCourseclassDone, AuditUserInfo },
  mounted () {
    this.search()
  },
    props: {
  ...
  methods: {
    ...
    handleAuditNext (record) {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          that.loading = true
          that.$post('dcaBDocCourseclass/updateNew', {
            jsonStr: jsonStr,
            state: 1
          }).then(() => {
            //this.reset()
    ...
    fetch (params = {}) {
      ...
      this.loading = true
      this.$get('dcaBDocCourseclass/audit', {
        ...params,
        state: 1
      }).then((r) => {
        let data = r.data
