  ...
  <div>
    <a-spin :spinning="loading">
      <a-card title="完成上一聘期">
        ...
        >
          <a-textarea
            :value="dcaBLastemploy.lastContent"
            @blur="e => inputChange(e.target.value,'lastContent')"
            :rows="16"
            placeholder="完成上一聘期"
          ></a-textarea>
          <a-textarea
            :value="dcaBLastemploy.auditSuggestion"
...
import DcaBLastemployDone from './DcaBLastemployDone'
...
export default {
  ...
  methods: {
    ...
    inputChange (value, filedName) {
      this.dcaBLastemploy[filedName] = value
    },
    onIsUseChange (e, filedName) {
      this.dcaBLastemploy[filedName] = e.target.checked;
    },
    OpenAudit (record) {
      this.dcaBLastemploy = record
    ...
    handleAudit () {
      ...
      this.$confirm({
        ...
        centered: true,
        onOk () {
          let jsonStr = JSON.stringify(that.dcaBLastemploy)
          that.loading = true
          that.$post('dcaBDocLastemploy/updateNew', {
            jsonStr: jsonStr,
            state: 3
          }).then(() => {
            //this.reset()
    ...
    fetch (params = {}) {
      ...
      this.loading = true
      this.$get('dcaBDocLastemploy/audit', {
        ...params,
        state: 1
      }).then((r) => {
        let data = r.data
