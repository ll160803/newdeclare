      ...
      >
        ...
        <div v-else>
          <a-select
            :value="record.khtype"
            style="width: 100%"
            @change="(e,f) => handleSelectChange(e,f,record,'khtype')"
          >
            <a-select-option value="中期考核">
              中期考核
            </a-select-option>
            <a-select-option value="出站考核">
              出站考核
            </a-select-option>
          </a-select>
        </div>
      ...
      >
        ...
        <div v-else>
          <a-date-picker
            ...
            @change="(e,f) => handleChange(e,f,record,'khdate')"
          />
        </div>
      </template>
      <template
  ...
  methods: {
    ...
    handleSave () {
      ...
      dataSource.forEach(element => {
        if (element.khjg != '' || element.khtype != '' || element.khdate != '' || element.adContent != '') {
      ...
      if (dataAdd.length === 0) {
        ...
        this.loading = true
        this.$post('dcaBDocAuditfiveother/addNew', {
          jsonStr: jsonStr,
          state: 0
        }).then(() => {
          // this.reset()
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          if (dataAdd.length === 0) {
            ...
            let jsonStr = JSON.stringify(dataAdd)
            that.loading = true
            that.$post('dcaBDocAuditfiveother/addNew', {
              jsonStr: jsonStr,
              state: 1
            }).then(() => {
              //this.reset()
    ...
    fetch () {
      this.$get('dcaBDocAuditfiveother/custom', {
      }).then((r) => {
        let data = r.data
