...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            setFormValues ({...dcaBSciencesearch}) {
                ...
                Object.keys(dcaBSciencesearch).forEach((key) => {
                    if (fields.indexOf(key) !== -1) {
                    this.form.getFieldDecorator(key)
                    let obj = {}
                    if(fieldDates.indexOf(key)!==-1)
                    {
                        if(dcaBSciencesearch[key]!=='') {
                            obj[key] = moment(dcaBSciencesearch[key])
                        }
                        else
                        {
                            obj[key] =''
                        ...
                        obj[key] = dcaBSciencesearch[key]
                    }
                    this.form.setFieldsValue(obj)
                }
            })
                this.dcaBSciencesearch. id                                   = dcaBSciencesearch. id                                              },
            handleSubmit () {
                this.form.validateFields((err, values) => {
                    if (!err) {
                    let dcaBSciencesearch = this.form.getFieldsValue()
                    dcaBSciencesearch. id                                  = this.dcaBSciencesearch.id
                    this.$put('dcaBDocSciencesearch', {
                        ...dcaBSciencesearch
                    }).then(() => {
                        this.reset()
                        this.$emit('success')
