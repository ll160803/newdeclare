      ...
      <a-card title="其他工作及成果">
        ...
        >
          <a-textarea
            ...
            @blur="e => inputChange(e.target.value,'otherWork')"
            :rows="16"
            placeholder="其他工作及成果"
          ></a-textarea>
          <a-textarea
            :value="dcaBOtherwork.auditSuggestion"
...
import DcaBOtherworkDone from './DcaBOtherworkDone'
...
export default {
  ...
  methods: {
    ...
    inputChange (value, filedName) {
      this.dcaBOtherwork[filedName] = value
    },
    onIsUseChange (e, filedName) {
      this.dcaBOtherwork[filedName] = e.target.checked;
    },
    OpenAudit (record) {
    ...
    handleAudit () {
      ...
      this.$confirm({
        ...
        centered: true,
        onOk () {
          let jsonStr = JSON.stringify(that.dcaBOtherwork)
          that.loading = true
          that.$post('dcaBDocOtherwork/updateNew', {
            jsonStr: jsonStr,
            state: 3
          }).then(() => {
            //this.reset()
    ...
    fetch (params = {}) {
      ...
      this.loading = true
      this.$get('dcaBDocOtherwork/audit', {
        ...params,
        state: 1
      }).then((r) => {
        let data = r.data
