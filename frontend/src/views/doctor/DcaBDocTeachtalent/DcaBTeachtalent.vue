<template>
  <a-card class="card-area" title="任现职以来完成教学、人才培养情况">
    <div>
      <a-button
        @click="handleAdd"
        type="primary"
  ...
  methods: {
    ...
    handleSave () {
      ...
      if (dataAdd.length < 0) {
        ...
        this.loading = true
        this.$post('dcaBDocTeachtalent/addNew', {
          jsonStr: jsonStr,
          state: 0
        }).then(() => {
          // this.reset()
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          if (dataAdd.length < 0) {
            ...
            let jsonStr = JSON.stringify(dataAdd)
            that.loading = true
            that.$post('dcaBDocTeachtalent/addNew', {
              jsonStr: jsonStr,
              state: 1
            }).then(() => {
              //this.reset()
    ...
    fetch () {
      this.$get('dcaBDocTeachtalent/custom', {
      }).then((r) => {
        let data = r.data
