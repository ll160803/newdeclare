...
import DcaBMedicalaccidentDone from './DcaBMedicalaccidentDone'
...
export default {
  ...
  components: { DcaBMedicalaccidentDone, AuditUserInfo },
  mounted () {
    this.search()
  },
  props: {
  ...
  methods: {
    ...
    freshTabs () {
        console.info(this.$refs.TableInfo3.queryParams)
    ...
    exportCustomExcel () {
      ...
      this.$export('dcaBDocMedicalaccident/excel', {
    ...
    handleAuditNext (record) {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          that.loading = true
          that.$post('dcaBDocMedicalaccident/updateNew', {
            jsonStr: jsonStr,
            state: 1
          }).then(() => {
            //this.reset()
    ...
    fetch (params = {}) {
      ...
      this.loading = true
      this.$get('dcaBDocMedicalaccident/audit', {
        ...params,
        state: 1
      }).then((r) => {
        let data = r.data
