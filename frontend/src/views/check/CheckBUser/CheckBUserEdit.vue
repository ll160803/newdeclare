...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            setFormValues ({...checkBUser}) {
                let fields = [  'ks'    , 'zjlb'      , 'sexName'    , 'birthday'    , 'telephone'    , 'idCard'    , 'zyjsgw'    , 'appointedDate'    , 'ksLeaderPernr'    , 'zgLeaderPernr'    , 'deptName'    , 'positionName'    , 'schoolDate'    , 'xcszyjzc'      , 'dcaYear'          ]
                let fieldDates = [        'birthday'               , 'schoolDate'              ]
                Object.keys(checkBUser).forEach((key) => {
                    ...
                    if(fieldDates.indexOf(key)!==-1)
                    {
                        ...
                        obj[key] = checkBUser[key]
                    }
                    this.form.setFieldsValue(obj)
                }
            })
                this.checkBUser. id                                                     = checkBUser. id                                                                },
            handleSubmit () {
                this.form.validateFields((err, values) => {
                    if (!err) {
                    let checkBUser = this.form.getFieldsValue()
                    checkBUser. id                                                    = this.checkBUser.id
                    this.$put('checkBUser', {
                        ...checkBUser
                    }).then(() => {
                        this.reset()
