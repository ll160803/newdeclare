  ...
  methods: {
    ...
    handleSave () {
      ...
      if (dataAdd.length < 0) {
        ...
        this.loading = true
        this.$post('dcaBUndergraduateprize/addNew', {
          jsonStr: jsonStr,
          state: 0
        }).then(() => {
          // this.reset()
    ...
    handleSubmit () {
      ...
      const dataSource = dataSourceAll.filter(p=>p.state==0 ||p.state==2)
          ...
          if (dataAdd.length < 0) {
            ...
            let jsonStr = JSON.stringify(dataAdd)
            that.loading = true
            that.$post('dcaBUndergraduateprize/addNew', {
              jsonStr: jsonStr,
              state: 1
            }).then(() => {
              //this.reset()
              ...
              that.$EventBus.$emit('selectMoudles',200)
            }, 300);
            }).catch(() => {
              that.loading = false
            })
    ...
    handleDelete () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          that.$put('dcaBUndergraduateprize', {
    ...
    fetch () {
      this.$get('dcaBUndergraduateprize/custom', {
      }).then((r) => {
        let data = r.data
