  ...
  methods: {
    ...
    handleSave () {
      ...
      dataSource.forEach(element => {
        if (element.fileId != '' || element.fileUrl != '' || element.tqCode != '' || element.tqReceiveDate != '') {
      ...
      if (dataAdd.length < 0) {
        ...
        this.loading = true
        this.$post('dcaBTeacherqualify/addNew', {
          jsonStr: jsonStr,
          state: 0
        }).then(() => {
          // this.reset()
    ...
    handleSubmit () {
      ...
      const dataSource = dataSourceAll.filter(p=>p.state==0 ||p.state==2)
          ...
          if (dataAdd.length < 0) {
            ...
            let jsonStr = JSON.stringify(dataAdd)
            that.loading = true
            that.$post('dcaBTeacherqualify/addNew', {
              jsonStr: jsonStr,
              state: 1
            }).then(() => {
              //this.reset()
              ...
              that.$EventBus.$emit('selectMoudles',140)
            }, 300);
            }).catch(() => {
              that.loading = false
            })
    ...
    fetch () {
      this.$get('dcaBTeacherqualify/custom', {
      }).then((r) => {
        let data = r.data
