    ...
    >
      ...
      >
        ...
        <div v-else>
          ...
          >
            <a-select-option value="校教学质量奖">
              校教学质量奖
            </a-select-option>
             <a-select-option value="校教学成果奖">
              校教学成果奖
            </a-select-option>
          </a-select>
        </div>
  ...
  methods: {
    ...
    handleSave () {
      ...
      if (dataAdd.length < 0) {
        ...
        this.loading = true
        this.$post('dcaBSchoolprize/addNew', {
          jsonStr: jsonStr,
          state: 0
        }).then(() => {
          // this.reset()
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          if (dataAdd.length < 0) {
            ...
            let jsonStr = JSON.stringify(dataAdd)
            that.loading = true
            that.$post('dcaBSchoolprize/addNew', {
              jsonStr: jsonStr,
              state: 1
            }).then(() => {
              //this.reset()
            ...
            //  that.fetch()
              ...
              that.$EventBus.$emit('selectMoudles',310)
            }, 300);
            }).catch(() => {
              that.loading = false
            })
    ...
    fetch () {
      this.$get('dcaBSchoolprize/custom', {
      }).then((r) => {
        let data = r.data
