  ...
  methods: {
    ...
    handleSave () {
      ...
      if (dataAdd.length < 0) {
        ...
        this.loading = true
        this.$post('dcaBTeacherprize/addNew', {
          jsonStr: jsonStr,
          state: 0
        }).then(() => {
          // this.reset()
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          if (dataAdd.length < 0) {
            ...
            let jsonStr = JSON.stringify(dataAdd)
            that.loading = true
            that.$post('dcaBTeacherprize/addNew', {
              jsonStr: jsonStr,
              state: 1
            }).then(() => {
              //this.reset()
             ...
             // that.fetch()
              ...
              that.$EventBus.$emit('selectMoudles',300)
            }, 300);
            }).catch(() => {
              that.loading = false
            })
    ...
    fetch () {
      this.$get('dcaBTeacherprize/custom', {
      }).then((r) => {
        let data = r.data
