  ...
  <a-card title="精品课程情况">
    ...
    >
      ...
      >
        ...
        <div v-else>
          <a-textarea
            @blur="e => inputChange(e.target.value,record,'course')"
            :value="record.course"
          >
          </a-textarea>
        </div>
      ...
      >
        ...
        <div v-else>
          <a-date-picker
            ...
            @change="(e,f) => handleChange(e,f,record,'coruseDate')"
          />
        </div>
      </template>
      <template
  ...
  methods: {
    ...
    handleSave () {
      ...
      dataSource.forEach(element => {
        if (element.course != '' || element.grade != '' || element.ranknum != '' || element.coruseDate != '') {
      ...
      if (dataAdd.length < 0) {
        ...
        this.loading = true
        this.$post('dcaBCourseclass/addNew', {
          jsonStr: jsonStr,
          state: 0
        }).then(() => {
          // this.reset()
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          if (dataAdd.length < 0) {
            ...
            let jsonStr = JSON.stringify(dataAdd)
            that.loading = true
            that.$post('dcaBCourseclass/addNew', {
              jsonStr: jsonStr,
              state: 1
            }).then(() => {
              //this.reset()
             ...
             // that.fetch()
              ...
              that.$EventBus.$emit('selectMoudles',320)
            }, 300);
            }).catch(() => {
              that.loading = false
            })
    ...
    fetch () {
      this.$get('dcaBCourseclass/custom', {
      }).then((r) => {
        let data = r.data
  ...
  computed: {
    columns () {
      ...
      },
      {
        ...
        width: 130,
        scopedSlots: { customRender: 'coruseDate' }
      },
      {
        title: '状态',
        dataIndex: 'state',
