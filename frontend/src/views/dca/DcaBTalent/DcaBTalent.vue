  ...
  <a-card class="card-area" title="任现职以来完成研究生教学人才培养情况">
    ...
    >
      ...
      >
        ...
        <div v-else>
          <a-textarea
            @blur="e => inputChange(e.target.value,record,'taletName')"
            :value="record.taletName"
          >
          </a-textarea>
        </div>
      </template>
      <template
        slot="talentStartDate"
        slot-scope="text, record"
      >
        <div v-if="record.state==3 || record.state==1">
        ...
        <div v-else>
          <a-date-picker
            ...
            @change="(e,f) => handleChange(e,f,record,'talentStartDate')"
          />
        </div>
      </template>
      <template
      ...
      >
        ...
        <div v-else>
          <a-date-picker
            :defaultValue="(text=='' || text==null)?'':moment(text, dateFormat)"
            @change="(e,f) => handleChange(e,f,record,'talentEndDate')"
          />
        </div>
      </template>
      <template
      ...
      >
        ...
        <div v-else>
          <a-textarea
            @blur="e => inputChange(e.target.value,record,'talentType')"
            :value="record.talentType"
          >
          </a-textarea>
        </div>
      </template>
  ...
  methods: {
    ...
    handleSave () {
      ...
      dataSource.forEach(element => {
        if (element.fileId != '' || element.fileUrl != '' || element.taletName != '' || element.talentStartDate != '' || element.talentEndDate != '' || element.talentType != '' || element.studentNumber != '' || element.totalTime != '' || element.personTime != '') {
          ...
          if (dataAdd.length < 0) {
            ...
            that.$post('dcaBTalent/addNew', {
             ...
             //  that.fetch()
              ...
              that.$EventBus.$emit('selectMoudles',210)
            }, 300);
            }).catch(() => {
              that.loading = false
            })
  ...
  computed: {
    columns () {
      return [
      {
        ...
        dataIndex: 'taletName',
        width: 130,
        scopedSlots: { customRender: 'taletName' }
      },
      {
        title: '自何年月',
        dataIndex: 'talentStartDate',
        width: 130,
        scopedSlots: { customRender: 'talentStartDate' }
      },
      {
        title: '至何年月',
        dataIndex: 'talentEndDate',
        width: 130,
        scopedSlots: { customRender: 'talentEndDate' }
      },
      {
        title: '课程类别',
        dataIndex: 'talentType',
        width: 130,
        scopedSlots: { customRender: 'talentType' }
      },
      {
        title: '学生人数',
