        ...
        <a-tabs type="card" @change="callback">
          <a-tab-pane key="1" tab="待审核">
            <a-table
              ...
              :scroll="scroll"
            >
              <template slot="letterDate" slot-scope="text, record">
                ...
                <div v-else>
                  <a-date-picker
                    ...
                    "
                    @change="(e, f) => handleChange(e, f, record, 'letterDate')"
                  />
                </div>
              </template>
              <template slot="letterJl" slot-scope="text, record">
                <div v-if="record.state == 3">
                  {{ text }}
                </div>
                <div v-else>
                  <a-textarea
                    @blur="
                      (e) => inputChange(e.target.value, record, 'letterJl')
                    "
                    :value="record.letterJl"
                  >
                  </a-textarea>
                </div>
              </template>
              <template slot="validDate" slot-scope="text, record">
                <div v-if="record.state == 3">
                  {{ text == "" || text == null ? "" : text.substr(0, 10) }}
                </div>
                <div v-else>
                  <a-date-picker
                    :defaultValue="
                      text == '' || text == null ? '' : moment(text, dateFormat)
                    "
                    @change="(e, f) => handleChange(e, f, record, 'validDate')"
                  />
                </div>
              </template>
              <template slot="auditState" slot-scope="text, record">
                ...
                <div v-else>
                  <a-input-number
                    @blur="
                      (e) => inputChange(e.target.value, record, 'auditState')
                    "
                    :value="record.auditState"
                    :precision="0"
                  >
                  </a-input-number>
                </div>
          ...
          <a-tab-pane key="2" tab="已审核" :forceRender="true">
            <dcaBLetter-done ref="TableInfo2" :state="3"> </dcaBLetter-done>
          </a-tab-pane>
          <a-tab-pane key="3" tab="审核未通过" :forceRender="true">
            <dcaBLetter-done ref="TableInfo3" :state="2"> </dcaBLetter-done>
          </a-tab-pane>
        </a-tabs>
        <audit-userInfo
          ref="userinfo"
...
import DcaBLetterDone from "./DcaBLetterDone";
...
export default {
  ...
  components: { DcaBLetterDone, AuditUserInfo },
  mounted() {
    this.search();
  },
  props: {
    ...
    columns() {
      return [
        ...
        },
        {
          ...
          width: 130,
          scopedSlots: { customRender: "letterDate" },
        },
        {
          title: "通讯评审结果",
          dataIndex: "letterJl",
          width: 130,
          scopedSlots: { customRender: "letterJl" },
        },
        {
          title: "有效期",
          dataIndex: "validDate",
