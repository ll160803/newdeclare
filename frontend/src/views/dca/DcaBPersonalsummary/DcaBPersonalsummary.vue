...
export default {
  ...
  methods: {
        ...
        onOk () {
          if (that.psContent.trim() === '') {
            ...
            that.$post('dcaBPersonalsummary', {
              ...
              that.$EventBus.$emit('selectMoudles',150)
            }, 300);
            }).catch(() => {
              that.loading = false
            })
    ...
    fetch () {
      this.$get('dcaBPersonalsummary/custom', {
      }).then((r) => {
        let data = r.data
