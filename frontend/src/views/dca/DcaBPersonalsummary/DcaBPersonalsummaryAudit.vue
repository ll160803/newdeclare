  ...
  <div>
    <a-spin :spinning="loading">
      <a-card title="个人总结">
        ...
        >
          <a-textarea
            :value="dcaBPersonalsummary.psContent"
            @blur="e => inputChange(e.target.value,'psContent')"
            :rows="16"
            placeholder="个人总结"
          ></a-textarea>
          <a-textarea
            :value="dcaBPersonalsummary.auditSuggestion"
          ...
          <a-checkbox
            ...
            :checked="dcaBPersonalsummary.isUse"
...
import DcaBPersonalsummaryDone from './DcaBPersonalsummaryDone'
...
export default {
  ...
  components: { DcaBPersonalsummaryDone },
  mounted () {
    this.fetch()
  },
  methods: {
    ...
    inputChange (value, filedName) {
      this.dcaBPersonalsummary[filedName] = value
    },
    onIsUseChange (e, filedName) {
      this.dcaBPersonalsummary[filedName] = e.target.checked;
    },
    OpenAudit (record) {
      this.dcaBPersonalsummary = record
    ...
    handleAudit () {
      ...
      this.$confirm({
        ...
        centered: true,
        onOk () {
          let jsonStr = JSON.stringify(that.dcaBPersonalsummary)
          that.loading = true
          that.$post('dcaBPersonalsummary/updateNew', {
            jsonStr: jsonStr,
            state: 3
          }).then(() => {
            //this.reset()
    ...
    fetch (params = {}) {
      ...
      this.loading = true
      this.$get('dcaBPersonalsummary/audit', {
        ...params,
        state: 1
      }).then((r) => {
        let data = r.data
