  ...
  methods: {
    ...
    handleSave () {
      ...
      dataSource.forEach(element => {
        if (element.fileId != '' || element.fileUrl != '' || element.spProjectName != '' || element.srProjectGrade != '' || element.srProjectLevel != '' || element.srPrizeDept != '' || element.srPrizeDate != '' || element.srPrizeRanknum != '') {
      ...
      if (dataAdd.length < 0) {
        ...
        this.loading = true
        this.$post('dcaBScientificprize/addNew', {
          jsonStr: jsonStr,
          state: 0
        }).then(() => {
          // this.reset()
    ...
    handleSubmit () {
      ...
      const dataSource = dataSourceAll.filter(p=>p.state==0 ||p.state==2)
          ...
          if (dataAdd.length < 0) {
            ...
            let jsonStr = JSON.stringify(dataAdd)
            that.loading = true
            that.$post('dcaBScientificprize/addNew', {
              jsonStr: jsonStr,
              state: 1
            }).then(() => {
              //this.reset()
             ...
             //  that.fetch()
              ...
              that.$EventBus.$emit('selectMoudles',100)
            }, 300);
            }).catch(() => {
              that.loading = false
            })
    ...
    fetch () {
      this.$get('dcaBScientificprize/custom', {
      }).then((r) => {
        let data = r.data
